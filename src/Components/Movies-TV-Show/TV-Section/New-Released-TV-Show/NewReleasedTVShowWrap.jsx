...
import NewReleasedTVSwiper from "./NewReleasedTVSwiper";

const NewReleasedTVShowWrap = () => {
  const { newReleasedTV, newReleasedDataLoading, error } = useSelector(
    (state) => state.newReleasedData
  );

  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(fetchNewReleasedMovieAndTV("tv"));
  }, []);

  const swiperRef = useRef(null);
  const slides = newReleasedTV || [];
