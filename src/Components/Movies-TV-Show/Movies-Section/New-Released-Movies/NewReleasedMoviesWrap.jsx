...
import { fetchNewReleasedMovieAndTV } from "../../../../Store/Actions/fetchNewReleasedMovieAndTV.action";
import SwiperNavigator from "../../../Common/MoviesAndTVStructure/SwiperNavigator";
import NewReleaseMoviesSwiper from "./NewReleaseMoviesSwiper";

const NewReleasedMoviesWrap = () => {
  const { newReleasedMovies, newReleasedDataLoading, error } = useSelector(
    (state) => state.newReleasedData
  );

  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(fetchNewReleasedMovieAndTV("movie"));
  }, []);

  const swiperRef = useRef(null);
  const slides = newReleasedMovies || [];
  ...
  return (
    <div className="w-full py-6 px-4">
      <div className="slideHeader flex w-full items-center h-15 justify-between">
        ...
        {!newReleasedDataLoading && totalGroups > 0 && (
          <SwiperNavigator
      ...
      <NewReleaseMoviesSwiper
        slides={slides}
        swiperRef={swiperRef}
        slidesPerGroup={slidesPerGroup}
        onSlideChange={handleSlideChange}
        loading={newReleasedDataLoading}
      />
    </div>
  );
};
